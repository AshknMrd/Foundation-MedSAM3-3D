...
def compute_dice(label_dir, results):

    label_jpg = PILImage.open(label_dir).convert("L") 
    label_array = np.array(label_jpg)
    binary_label = (label_array > 127).astype(int)  

    masks = results[0]['masks']
    binary_pred = np.any(masks, axis=0).astype(int)

    intersection = np.sum(binary_label & binary_pred)
    dice_score = (2 * intersection) / (np.sum(binary_label) + np.sum(binary_pred) + 1e-6)
    return dice_score
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to SAM3 weights, locally downloaded"
    )
    parser.add_argument(
        "--weights",
        type=str,
    ...
    if args.label is not None:
        dice_score = compute_dice(args.label, results)
        print(f"\n Dice score for GT label and the prediction is: {dice_score:.4f}")
